...
const SIDE_TAB_TRIGGER_CLASS_NAME = "h-auto flex-none cursor-pointer rounded-none border-0 border-b-2 border-transparent !bg-transparent px-0 pb-2 shadow-none after:hidden data-[state=active]:border-primary data-[state=active]:!bg-transparent data-[state=active]:shadow-none";
...
type ContextMenuDemoPageProps = {
  searchParams: Promise<{ article?: string | string[] }>;
};

export default function ContextMenuDemoPage({ searchParams }: ContextMenuDemoPageProps) {
  const query = use(searchParams);
  const articleParam = Array.isArray(query.article) ? query.article[0] : query.article;

  if (!articleParam) {
    return <IeltsArticleList />;
  }
...
function IeltsArticleList() {
  return (
    <main className="min-h-svh px-6 py-10">
      <section className="mx-auto flex w-full max-w-3xl flex-col gap-7">
        <div className="space-y-2 border-b pb-5">
          <p className="text-sm font-medium text-muted-foreground">ITLES</p>
          <h1 className="text-3xl font-semibold tracking-normal">Reading Articles</h1>
        </div>
          ...
          {getArticleList(allArticles).map((article) => (
            ...
            >
              <div className="flex min-w-0 gap-3">
                <span className="mt-1 flex size-8 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground group-hover:text-primary">
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-stretch">
          <section className="min-w-0 flex-[7]">
            <header className="mb-7 space-y-3">
              ...
              <h1 className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-3xl font-semibold tracking-normal text-balance pl-8">
                {article.title}
